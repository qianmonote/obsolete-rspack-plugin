...
| `zIndex` | `number` | — | 提示条容器的 `z-index` 值，设置后自动添加 `position:fixed;top:0;left:0;right:0` 使其固定在页面顶部 |
...
| `closable` | `boolean \| object` | `false` | 是否显示关闭按钮。`true` 使用默认样式（`×` 圆形按钮），传入对象可自定义：`{ text, ariaLabel, className, style }`（见下方说明）。对默认和自定义 `template` 均生效；若 `template` 中已有 `id="obsoleteClose"` 则不重复注入 |
...
> | 属性 | 类型 | 默认值 | 说明 |
> | :-- | :-: | :-: | :-- |
> | `text` | `string` | `'&times;'` | 按钮内的 HTML 内容 |
> | `ariaLabel` | `string` | `'Close'` | 无障碍标签 |
> | `className` | `string` | `''` | 追加到按钮上的 CSS 类名 |
> | `style` | `string` | `''` | 追加到按钮的内联 CSS，如 `'right:20px;font-size:20px'` |
...
```html
<div id="obsoleteContainer" style="position:fixed;top:0;left:0;right:0;z-index:99999">
  <div>Your browser is not supported.</div>
</div>
```
...
<div id="obsoleteContainer"
     style="position:fixed;top:0;left:0;right:0;z-index:99999;background:#fff3cd;color:#856404;padding:12px 20px;font-size:14px;text-align:center;border-bottom:1px solid #ffc107"
...
默认 `closable: false`，不显示关闭按钮。设为 `true` 后自动注入带样式的 `×` 圆形按钮：
...
自定义关闭按钮 — 传入对象覆盖文字、样式、类名：

```js
new ObsoleteRspackPlugin({
  closable: {
    text: 'Dismiss',                              // 按钮文字
    style: 'right:20px;width:auto;padding:2px 14px;border-radius:4px;font-size:12px',
    className: 'my-close-btn',                     // 自定义类名
  },
  zIndex: 99999,
})
```

> `closable` 对默认模板和自定义 `template` 均生效。如果 `template` 中已包含 `id="obsoleteClose"` 的元素，则不会重复注入按钮。
...
A: 不会。两者自动合并为一个 `style` 属性，`zIndex` 转换的 `position:fixed;top:0;left:0;right:0;z-index:N` 在前，`containerStyle` 的值追加在后。如果 `containerStyle` 中也写了 `z-index` 或 `position`，后者会覆盖前者（CSS 特性）。
...
A: 有效。`closable` 对默认和自定义模板均生效 — 如果模板中没有 `id="obsoleteClose"` 的元素，插件会自动注入一个带样式的关闭按钮。如果模板中已有该元素，则不会重复注入。
